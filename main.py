...
def start_increasing_iop_values(working_row, iop_index, row_index, rhas, computed_values_dict, delete_memory_index,
    ...
    if current_row_computed_values:
        ...
        logging.info(
            f"ppi---->{parent_pipe_index}==dmi---->{delete_memory_index} &&&& {previous_row['iop_index']} == {highest_iop_index_of_first_pipe} ")
        if parent_pipe_index == delete_memory_index and previous_row[
            'iop_index'] == highest_iop_index_of_first_pipe:
            ...
            if parent_pipe_index != 0:
                computed_values = start_increasing_iop_values(previous_row['row_vals'], highest_iop_index_of_first_pipe,
                                                              parent_pipe_index, previous_row['rhas'], computed_values_dict,
                                                              delete_memory_index, highest_iop_index_of_first_pipe)
            else:
                computed_values = start_increasing_iop_values(previous_row['row_vals'], previous_row['iop_index'] + 1,
                                                              parent_pipe_index, previous_row['rhas'],
